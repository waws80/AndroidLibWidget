package pw.androidthanatos.lib.widget.pickerview;
...
/**
 *  @desc: 自定义滚轮view
 *  @className: NumberPickerView
 *  @author: thanatos
 *  @createTime: 2018/12/19
 *  @updateTime: 2018/12/19 3:36 PM
 */
public class NumberPickerView extends View{
    ...
    private void initHandler(){
        ...
        mHandlerInNewThread = new HandlerInNewThread(mHandlerThread.getLooper(),this);
        mHandlerInMainThread = new HandlerInMainThread(this);
    }

    static class HandlerInNewThread extends Handler{

        private WeakReference<NumberPickerView> pickerViewWeakReference;

        HandlerInNewThread(Looper looper, NumberPickerView pickerView){
            super(looper);
            pickerViewWeakReference = new WeakReference<>(pickerView);
        }

        @Override
        public void handleMessage(Message msg) {
            if (pickerViewWeakReference == null || pickerViewWeakReference.get() == null){
                return;
            }
            switch(msg.what){
                case HANDLER_WHAT_REFRESH:
                    if(!pickerViewWeakReference.get().mScroller.isFinished()){
                        if(pickerViewWeakReference.get().mScrollState == OnScrollListener.SCROLL_STATE_IDLE){
                            pickerViewWeakReference.get().onScrollStateChange(OnScrollListener.SCROLL_STATE_TOUCH_SCROLL);
                        }
                        pickerViewWeakReference.get().mHandlerInNewThread.sendMessageDelayed(pickerViewWeakReference.get().getMsg(HANDLER_WHAT_REFRESH, 0, 0, msg.obj), HANDLER_INTERVAL_REFRESH);
                    }else{
                        int duration = 0;
                        int willPickIndex;
                        //if scroller finished(not scrolling), then adjust the position
                        if(pickerViewWeakReference.get().mCurrDrawFirstItemY != 0){//need to adjust
                            if(pickerViewWeakReference.get().mScrollState == OnScrollListener.SCROLL_STATE_IDLE){
                                pickerViewWeakReference.get().onScrollStateChange(OnScrollListener.SCROLL_STATE_TOUCH_SCROLL);
                            }
                            if(pickerViewWeakReference.get().mCurrDrawFirstItemY < (-pickerViewWeakReference.get().mItemHeight/2)){
                                //adjust to scroll upward
                                duration = (int)((float)DEFAULT_INTERVAL_REVISE_DURATION * (pickerViewWeakReference.get().mItemHeight + pickerViewWeakReference.get().mCurrDrawFirstItemY) / pickerViewWeakReference.get().mItemHeight);
                                pickerViewWeakReference.get().mScroller.startScroll(0, pickerViewWeakReference.get().mCurrDrawGlobalY, 0, pickerViewWeakReference.get().mItemHeight + pickerViewWeakReference.get().mCurrDrawFirstItemY, duration * 3);
                                willPickIndex = pickerViewWeakReference.get().getWillPickIndexByGlobalY(pickerViewWeakReference.get().mCurrDrawGlobalY + pickerViewWeakReference.get().mItemHeight + pickerViewWeakReference.get().mCurrDrawFirstItemY);
                            }else{
                                //adjust to scroll downward
                                duration = (int)((float)DEFAULT_INTERVAL_REVISE_DURATION * (-pickerViewWeakReference.get().mCurrDrawFirstItemY) / pickerViewWeakReference.get().mItemHeight);
                                pickerViewWeakReference.get().mScroller.startScroll(0, pickerViewWeakReference.get().mCurrDrawGlobalY, 0, pickerViewWeakReference.get().mCurrDrawFirstItemY, duration * 3);
                                willPickIndex = pickerViewWeakReference.get().getWillPickIndexByGlobalY(pickerViewWeakReference.get().mCurrDrawGlobalY + pickerViewWeakReference.get().mCurrDrawFirstItemY);
                            }
                            pickerViewWeakReference.get().postInvalidate();
                        }else{
                            pickerViewWeakReference.get().onScrollStateChange(OnScrollListener.SCROLL_STATE_IDLE);
                            //get the index which will be selected
                            willPickIndex = pickerViewWeakReference.get().getWillPickIndexByGlobalY(pickerViewWeakReference.get().mCurrDrawGlobalY);
                        }
                        Message changeMsg = pickerViewWeakReference.get().getMsg(HANDLER_WHAT_LISTENER_VALUE_CHANGED, pickerViewWeakReference.get().mPrevPickedIndex, willPickIndex, msg.obj);
                        if(pickerViewWeakReference.get().mRespondChangeInMainThread){
                            pickerViewWeakReference.get().mHandlerInMainThread.sendMessageDelayed(changeMsg, duration * 2);
                        }else{
                            pickerViewWeakReference.get().mHandlerInNewThread.sendMessageDelayed(changeMsg, duration * 2);
                        }
                    }
                    break;
                case HANDLER_WHAT_LISTENER_VALUE_CHANGED:
                    pickerViewWeakReference.get().respondPickedValueChanged(msg.arg1, msg.arg2, msg.obj);
                    break;
            }
        }
    }
    ...
    static class HandlerInMainThread extends Handler{
        ...
        HandlerInMainThread(NumberPickerView pickerView){
            pickerViewWeakReference = new WeakReference<>(pickerView);
        }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what){
                case HANDLER_WHAT_REQUEST_LAYOUT:
                    pickerViewWeakReference.get().requestLayout();
                    break;
                case HANDLER_WHAT_LISTENER_VALUE_CHANGED:
                    pickerViewWeakReference.get().respondPickedValueChanged(msg.arg1, msg.arg2, msg.obj);
                    break;
